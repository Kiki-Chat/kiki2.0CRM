              ...
              return (
                <div key={s.id} className="flex items-start gap-3 rounded-lg border border-border p-3">
                  ...
                  <div className="flex shrink-0 items-center gap-1.5">
                    ...
                    <button onClick={() => act.mutate({ suggestion_key: s.id, action: 'snooze', snooze_days: 3 })} className="rounded-md px-2.5 py-1.5 text-xs font-medium text-muted hover:bg-alt">3 Tage später erinnern</button>
                  </div>
                </div>
              )
            })}
