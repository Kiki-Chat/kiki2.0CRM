...
def _render_appointment_reschedule(record: dict, customer: dict | None, org: dict) -> Rendered:
    ...
    if alt:
        ...
        first = (
            ...
            f"um {de_time(alt)} Uhr verschieben. Würde Ihnen das passen?"
        )
        voicemail = (
            ...
            f"{de_time(alt)} Uhr. Melden Sie sich gerne bei uns. Auf Wiederhören!"
        )
        task = (
    ...
    first = (
        ...
        "gelegt. Passt Ihnen der neue Termin?"
    )
    voicemail = (
        f"Guten Tag, hier ist der Telefonassistent von {company}. Wir mussten Ihren Termin{fuer} "
        f"verschieben und haben ihn neu auf {datum} um {uhr} Uhr gelegt. Falls der neue Termin "
        "nicht passt, melden Sie sich gerne bei uns. Auf Wiederhören!"
    )
    task = (
