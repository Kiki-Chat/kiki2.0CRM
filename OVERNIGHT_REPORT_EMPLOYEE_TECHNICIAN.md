...
- **UAT Railway deploy — was serving stale code, now FIXED.** Root cause: `railway up` from a git **worktree** uploaded a stale cached snapshot (build SUCCEEDED but shipped old code — the build log showed a 5-step Dockerfile vs the committed 6-step, i.e. the upload itself was stale). Fix: deploy from a clean `git archive` copy — `git archive HEAD | tar -x -C <tmp>`, then from inside `<tmp>`: `railway up --ci -p <projectId> -s backend -e production`. The fresh build showed 6 steps and the new route went live (`/api/technician/me/jobs` → 401, openapi 245→247 paths). Backend is live; frontend re-deployed the same way. **To deploy this branch in future, deploy from a clean non-worktree copy, not the worktree directly.**
