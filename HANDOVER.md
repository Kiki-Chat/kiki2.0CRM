...
1. **Finanzen → Rechnungen** (invoices): the `invoices` table exists but no UI/endpoints. The KVA "in Rechnung umwandeln" action sets status='invoiced' but doesn't yet create an invoice. Mirror the KVA module.
...
- **Email not configured**: employee invite (`invite_user_by_email`) and KVA `send` don't actually email until Supabase SMTP/templates are set up. Invite gracefully creates a no-login employee + warning; use the 🔑 **set-password** action to give a working login. KVA `send` just marks status='sent'.
...
- Invoices, Projekte/projects table, Kiki-Zentrale, Settings not built.
