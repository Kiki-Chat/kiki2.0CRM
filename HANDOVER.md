...
10. **`SETTINGS_ENC_KEY` is REQUIRED** (Fernet key encrypting stored SMTP/PDS creds). The backend **raises at import** if it's unset — set it in `backend/.env` (local) AND the Railway env vars (prod), using the **same value** so prod can decrypt what local wrote. Generate: `python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"`. `.env.example` has a placeholder, not the real key.

## Credentials (live in gitignored .env files — read them, don't guess)
- `backend/.env` — `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `MASTER_WEBHOOK_SECRET`, `POST_CALL_WEBHOOK_SECRET`, `ELEVENLABS_API_KEY`, `SETTINGS_ENC_KEY` (values live in the gitignored file, not here).
...
- **0013** projects (Projekte): new tables **projects** (`number` PRJ-YYYY-#####, `title`, `status` planning|active|completed|archived, `start_date`/`end_date`, `planned_budget`, `project_address` jsonb {street,postcode,city}, `internal_notes`, `notes_updated_at`, `created_by`) + **project_employees** (project↔employee, unique). Added `project_id` to **inquiries/appointments/cost_estimates/invoices/documents**. RLS org policies; backend uses service role.
- **0014** settings (Einstellungen): `organizations.{fax,website,management jsonb,chamber_of_crafts,google_reviews_enabled}`; **`public.users.language_preference`** (text not null default 'de'); `agent_configs.{ai_suggestions_enabled,kva_followup_days,payment_reminder_days,appointment_reminder_days,maintenance_reminder_days}`; new tables **email_configs** + **pds_configs** (encrypted creds, org-unique, RLS). Plus public storage bucket **`org-assets`** (logos).
...
- Projects (Projekte): `GET/POST /api/projects` (list returns per-project `stats`/`progress`/`actual_budget`/`open_amount`; filters status/customer_id/search), `GET/PATCH/DELETE /{id}` (DELETE = soft archive; GET is rich: stats, employees, next_appointment, open invoices), `GET /{id}/activity` (merged chronological timeline), sub-resources `GET /{id}/{calls,inquiries,appointments,cost-estimates,invoices,documents,employees}` + `POST /{id}/documents` (uploads to `projects/{id}/`), team `POST /{id}/employees` + `DELETE /{id}/employees/{employee_id}`. `project_id` now accepted by POST cost-estimates/invoices/appointments/inquiries and PATCH inquiries. Routes: `app/api/routes/projects.py`; numbering `app/services/projects.py`.
- Settings (Einstellungen): `GET /api/settings` (org + email/pds config [ciphertext omitted, returns `has_password`/`has_api_key`] + ai-suggestion thresholds + `usage`{ai_minutes_used/quota, active_employees, document_count/size}); `PATCH /api/settings/{general,design,ai-suggestions,google-reviews,email-config,pds-config}`; `POST /api/settings/{logo,email-test,pds-test,pds-sync,generate-suggestions}`; `DELETE /api/settings/{logo,organization}` (org delete requires role=org_admin + header `X-Confirm-Delete: {orgName}`). SMTP password + PDS api key encrypted at rest (Fernet, `app/core/crypto.py`). General PATCH replaces `address` wholesale (drops legacy `{raw}`). Routes: `app/api/routes/settings.py`.
- Users: `GET/PATCH /api/users/me` (full_name, language_preference de|en), `POST /api/users/me/change-password` (verifies current via throwaway sign-in, sets new via Supabase admin API). `app/api/routes/users.py`.
...
- **Projekte** (`/projects`): list (cards w/ status badge, customer link, number, date range, progress bar [completed/total appts], 6-icon stats, Geplant/Ist budget; filters search/status/customer/Zeitraum; 3 summary cards Aktive/Gesamtbudget/Offene Posten) · create/edit form (`/projects/new`, `/:id/edit`: Grunddaten, Budget, Projektadresse [+ "Kundenadresse übernehmen"], Interne Notizen) · **workspace** (`/projects/:id`) — **persistent left sidebar** + 9 tabs: Übersicht (4 stat cards, budget bar, activity feed) · Anrufe · Anfragen (expandable, + neu, + bestehende verknüpfen) · Termine (mini FullCalendar + list, + neu) · Kostenvoranschläge · Rechnungen (Gesamt/Bezahlt/Offen, mark-paid, Zahlungserinnerung toast) · Team (add/remove, "X Termine übernommen") · Dokumente (drag-drop upload) · Notizen (auto-save on blur + local Verlauf). Header fetched once; all sub-resource queries `staleTime: 5min`; tab = local state (sidebar never unmounts). `ProjectsPage.tsx`, `ProjectFormPage.tsx`, `ProjectWorkspacePage.tsx`, `projectTabs.tsx`. Create-from-project deep links: `?project_id=&customer_id=` on KVA/invoice forms; KVA→invoice carries `&project_id=`.
- **Kalender** now has a "Termine | Projekt-Timeline" toggle (`CalendarPage.tsx`): timeline renders projects as month spanning bars colored by status (planning gray/active green/completed blue/archived muted); click → `/projects/:id`. Appointment-only controls hide in timeline mode.
- **Einstellungen** (`/settings/:section`, `/settings` → redirect `stammdaten`): two-column (240px sub-nav + only the active section), deep-linkable. Sections: Stammdaten · Design · Abrechnung · KI-Vorschläge · Benachrichtigungen · E-Mail-Versand · E-Mail-Vorlagen · Kalender-Sync · Google Reviews · PDS-Software · Gefahrenzone. One primary green save per section; OAuth/onboarding-reset/pds-test+sync are stubs (toast + TODO). `SettingsPage.tsx`. **Accent theming**: `lib/accent.ts applyAccent()` overrides the `--green-*` vars from `org.accent_color` — applied on app load in `AppLayout` and live on swatch pick (no hardcoded green).
- **Persönliche Einstellungen** modal (`components/PersonalSettingsModal.tsx`, from the sidebar profile dropdown): Profil (name, read-only email) · Sprache DE/EN (→ `users.language_preference`, also sets client `useLang`) · Darstellung (dark toggle via `useTheme`, persists in `localStorage['hk-theme']`) · Passwort ändern. Dropdown now: "Firmeneinstellungen" → `/settings`, "Persönliche Einstellungen" → modal.
...
2. ~~**Projekte** + **Project timeline**~~ **DONE** — migration 0013, project endpoints, `/projects` list + create/edit form + workspace (9 tabs), and the Calendar Projekt-Timeline. See above.
...
- Kiki-Zentrale + remaining Dashboard tabs (Calls/Finance/Time/AI) not built. (**Rechnungen** 0012, **Projekte** 0013, **Einstellungen** 0014 now built.)
- Settings stubs (toast + TODO, no backend yet): Gmail/Outlook/Calendly OAuth, onboarding-reset, PDS test/sync, avatar upload, ServiceWorker push subscription.
- Test org `c4dbf596` also has 2 seeded **projects** (PRJ-2026-00001 Fassadenrenovierung Müller [active], PRJ-2026-00002 Heizungstausch Sharma [planning]) with linked KVAs/invoices/inquiries/appointments + one project_employee. Projekte tab actions that "log activity" (e.g. Zahlungserinnerung) are toast-only (no reminders table). NotesTab Verlauf is local-state only (not persisted).
